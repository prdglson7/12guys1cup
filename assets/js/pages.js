  ...
  try {
    ...
    for (let w = 1; w <= currentWeek; w++) {
      try {
        ...
        t.filter(x => x.status === 'complete').forEach(x => allTxns.push(x));
      } catch (_) {}
    }
...
function buildDepthChartOverlay(allPlayers, sleeperPlayers) {
  ...
  // FantasyPros injuries (from p._injury set by injury enrichment)
  allPlayers.forEach(p => {
    if (p.name && p._injury) {
